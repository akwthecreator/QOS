...
function paintAll() {
  ...
  _secs.forEach(b => { if (b._sec.kind === 'sheets') paintSec(b); });
...
function paintSec(box) {
  ...
  if (sec.kind === 'sheets') {
    const on = canSwitch();
    const opts = optsFor(sec);
    wrap.innerHTML = opts.map(t =>
      `<button type="button" class="qcp-chip${t === _cfg[sec.field] ? ' on' : ''}"
        data-v="${esc(t)}" ${on ? '' : 'disabled'}>${esc(t)}</button>`).join('')
      || '<div class="qcp-sect" style="padding:0 4px">Листы не загружены</div>';
    if (!on) return;
    wrap.querySelectorAll('.qcp-chip').forEach(b => {
      b.onclick = () => { const p = {}; p[sec.field] = b.dataset.v; set(p); };
    });
    return;
  }
